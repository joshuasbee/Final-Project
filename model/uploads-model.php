...
function getThumbnails($invId) {
  $db = phpmotorsConnect();
  $sql = 'SELECT images.imgPath, inventory.invMake, inventory.invModel FROM images INNER JOIN inventory ON images.invId = inventory.invId WHERE images.invId = :invId AND images.imgPath LIKE "%-tn.%"';
  $stmt = $db->prepare($sql);
  $stmt->bindValue(':invId', $invId, PDO::PARAM_STR);
  $stmt->execute();
  $tn = $stmt->fetchAll(PDO::FETCH_ASSOC);
  $stmt->closeCursor();
  return $tn;
}